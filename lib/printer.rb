...
class Printer
  ...
  #attr_reader :welcome_message, :game_prompt, :turn_response
...
(g)reen, (b)lue, and (y)ellow. Use (q)uit at any time to end the game.\n What's your guess?"
  ...
  def time_message(solution, guess_count, minutes, seconds)
    puts "Congratulations! You guessed the sequence '#{solution.upcase}' in #{guess_count} guesses over #{minutes} minutes and #{seconds} seconds. Do you want to (p)lay again or (q)uit?"
  end
  ...
  def correct_place(guess, correct_color, correct_place)
    puts "'#{guess}' has #{correct_color} of the correct elements with #{correct_place} in the correct position"
  end
end
